...
def _write_split_rules(rules: list[dict], output_dir: str, stats: CompileStats) -> None:
    ...
    # Major sites whose sub_frame/main_frame resources must not be broken
    # by broad annoyance rules. These sites handle their own ad removal
    # via dedicated content scripts (e.g. youtube-blocker.js).
    ANNOYANCE_SAFE_DOMAINS = [
        "youtube.com", "google.com", "gstatic.com", "ytimg.com",
        "googlevideo.com", "googleapis.com", "youtube-nocookie.com",
    ]
    ...
    # Protect major sites from annoyance rules: add excludedInitiatorDomains
    # for broad rules (those without initiatorDomains/requestDomains restrictions)
    for rule in annoyances:
        cond = rule.get("condition", {})
        if cond.get("initiatorDomains") or cond.get("requestDomains"):
            continue  # Already domain-scoped, leave as-is
        existing_excl = set(cond.get("excludedInitiatorDomains", []))
        existing_excl.update(ANNOYANCE_SAFE_DOMAINS)
        cond["excludedInitiatorDomains"] = sorted(existing_excl)
